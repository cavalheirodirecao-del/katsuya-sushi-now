...
const Menu = () => {
  ...
  // Map category name to the category id used in products table
  const getCategorySlug = (name: string) =>
    name.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/\s+/g, "-").replace(/ç/g, "c");

  // Find matching products - try slug match first, then direct name match
  const items = activeCategories.length > 0
    ? getByCategory(
        activeCategory ||
        getCategorySlug(activeCategories[0].name)
      )
    : [];
