...
import { Building2, Phone, MapPin, Instagram, Facebook, Clock, Image, Loader2, Save } from "lucide-react";
      ...
      <section className="bg-card border border-border rounded-xl p-4 space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <Input
            ...
            placeholder="Ex: Katsuya Sushi Delivery"
          />
        </div>
        ...
        <div className="space-y-1.5">
          <Label className="flex items-center gap-1.5">
            <Image className="h-3.5 w-3.5" /> URL do Logo
          </Label>
          <Input
            type="url"
            value={form.logo_url || ""}
      ...
      <section className="bg-card border border-border rounded-xl p-4 space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-xs text-muted-foreground">
            Formato: código do país + DDD + número (ex: 5581982522785)
          </p>
          <div className="flex items-center gap-2 mt-1 p-2 bg-primary/10 border border-primary/30 rounded-lg">
            <Phone className="h-3.5 w-3.5 text-primary" />
            <p className="text-xs text-primary font-medium">
              Pedidos do catálogo serão enviados para: wa.me/{form.phone.replace(/\D/g, "") || "..."}
            </p>
          </div>
        </div>
